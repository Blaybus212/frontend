...
export default function ViewerPage() {
  ...
  return (
    <div className="h-full w-full relative overflow-hidden bg-surface">
      ...
      {/* 씬 로딩 중 스피너: 씬 위에 절대 배치 */}
      {sceneIdParam && !activeModelUrl && (
        <div
          className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/60"
          style={{ right: `${effectiveRightPanelWidth}%` }}
        >
          <svg
            className="mb-4 h-12 w-12 animate-spin text-point-500"
          ...
          <span className="text-b-md font-weight-medium text-white">씬 준비 중입니다</span>
        </div>
      )}
